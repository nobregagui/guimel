...
export interface ProdutoActionMenuItem {
  id: string
  label: string
  onClick: () => void
  disabled?: boolean
...
interface ProdutoActionMenuProps {
  items: ProdutoActionMenuItem[]
  ariaLabel?: string
}

export function ProdutoActionMenu({ items, ariaLabel = 'Ações' }: ProdutoActionMenuProps) {
  ...
  const handleSelect = useCallback((item: ProdutoActionMenuItem) => {
