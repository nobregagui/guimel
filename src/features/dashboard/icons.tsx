...
export function NavIcon({ icon, className, size = defaults.size }: { icon: SidebarNavItem['icon'] } & IconProps) {
  const props = { className, width: size, height: size, viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', strokeWidth: 2, strokeLinecap: 'round' as const, strokeLinejoin: 'round' as const }
...
export function MiniSparkline({ points, color, className }: { points: number[]; color: string; className?: string }) {
  const w = 120
  const h = 36
  const pad = 2
